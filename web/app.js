...
async function loadTagsTab() {
  try {
    ...
    if (byCat.size === 0) {
      container.innerHTML =
        '<div class="card"><p class="muted">Noch keine Tags. Tags werden beim Scan automatisch vergeben.</p></div>';
      return;
    }
    ...
    document.querySelectorAll('#tags-by-category .word.tag').forEach((el) => {
      ...
      el.addEventListener('click', () => {
        ...
        document
          ...
          .forEach((b) => b.classList.toggle('active', b.dataset.tab === 'articles'));
        document
  ...
  switch (type) {
    ...
    case 'scan-complete':
      $('#scan-status').textContent =
        `Fertig: ${payload.summary.articlesAdded} neue Artikel, ${payload.summary.duplicatesFound} Duplikate, ${payload.summary.errors} Fehler`;
  ...
  try {
    ...
    $('#filter-source').innerHTML =
      ...
      sources
        .map(
          (s) =>
            `<option value="${escapeHtml(s)}"${s === cur ? ' selected' : ''}>${escapeHtml(s)} (${data.articles.filter((a) => a.source === s).length})</option>`
        )
        .join('');

    renderArticleList(data.articles);
    renderActiveFilterChips();
    $('#articles-subtitle').textContent =
      `${data.returned} von ${data.total} Artikeln · Zeitraum ${fmtDate(data.from)} – ${fmtDate(data.to)}`;
  } catch (err) {
    toast(err.message, 'error');
  }
}
...
function renderArticleList(articles) {
  ...
  $('#results-count').innerHTML =
    `<strong>${sorted.length}</strong> Artikel${state.search ? ` für „${escapeHtml(state.search)}“` : ''}`;
  if (!sorted.length) {
    list.innerHTML = '';
...
async function showArticleDetail(id) {
  try {
    ...
    const alsoOn = a.also_on
      ? typeof a.also_on === 'string'
        ? JSON.parse(a.also_on)
        : a.also_on
      : [];
    ...
    $$('#md-tags .x').forEach((x) =>
      x.addEventListener('click', async () => {
        try {
          await api(`/api/article/${a.id}/tags/${encodeURIComponent(x.dataset.tag)}`, {
            method: 'DELETE',
          });
          dlg.close();
          showArticleDetail(id);
...
function initArticleFilters() {
  ...
  $('#article-search').addEventListener('input', (e) => {
    ...
    if (state.search) {
      relOpt.disabled = false;
      $('#filter-sort').value = 'relevance';
      state.filters.sort = 'relevance';
    } else {
      state.filters.sort = 'score-desc';
  ...
  $('#filter-sentiment').addEventListener('change', (e) => {
  ...
  $('#filter-bookmark').addEventListener('change', (e) => {
    state.filters.bookmark = e.target.value;
    loadArticles();
  });
...
async function addTagToArticle(articleId, tag) {
  ...
  try {
    await api(`/api/article/${articleId}/tags`, {
      method: 'POST',
  ...
  try {
    ...
    if (!health.length) {
      $('#feed-health').innerHTML =
        '<p class="muted">Noch keine Health-Daten. Führe einen Scan aus.</p>';
    } else {
      const sorted = [...health].sort(
        (a, b) =>
          safeNum(b.consecutive_failures) - safeNum(a.consecutive_failures) ||
  ...
  try {
    ...
    $('#reports-list').innerHTML =
      ...
      `
            ...
            .join('');
    $$('[data-action="open"]').forEach((b) =>
      b.addEventListener('click', () => window.open(`/api/reports/${b.dataset.name}`, '_blank'))
    );
    $$('[data-action="delete"]').forEach((b) =>
...
async function loadKeywords() {
  try {
    ...
    const labels = {
      required: [
        'Pflicht-Begriffe (mindestens einer muss vorkommen)',
        'Mindestens einer dieser Begriffe muss im Titel oder Text auftauchen',
      ],
...
function renderHealthStats() {
  const stats = (state.sources && state.sources.stats) || {
    ok: 0,
    degraded: 0,
    blocked: 0,
    dead: 0,
  ...
  el.innerHTML =
    pills +
    `<button type="button" class="health-pill total${!state.sourcesFilter ? ' active' : ''}" data-filter="">
    ...
    $('.src-test', row).addEventListener('click', async () => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          const httpBadge = r.statusCode
            ? `<span class="badge badge-outline">HTTP ${r.statusCode}</span>`
            : '';
          const browserBadge = r.viaBrowser
            ? '<span class="badge badge-blocked">via Browser</span>'
            : '';
          const parts = [
          ...
          const httpBadge = r.statusCode
            ? `<span class="badge badge-outline">HTTP ${r.statusCode}</span>`
            : '';
          const clsBadge = r.errorClass
            ? `<span class="badge badge-warn">${escapeHtml(r.errorClass)}</span>`
            : '';
...
function initSourcesTab() {
  ...
  if (opmlPreviewBtn) {
    opmlPreviewBtn.addEventListener('click', async () => {
      ...
      try {
        const r = await api('/api/sources/opml-preview', {
          method: 'POST',
          body: JSON.stringify({ opml }),
        });
        const rows = (r.previews || [])
          .map((p) => {
            const cls =
              p.level === 'ok'
                ? 'opml-row-ok'
  ...
  if (opmlImportBtn) {
    opmlImportBtn.addEventListener('click', async () => {
      ...
      if (!opml) return toast('OPML-Inhalt einfügen', 'error');
      try {
        const r = await api('/api/sources/opml-import', {
          method: 'POST',
          body: JSON.stringify({ opml }),
        });
  ...
  if (btn) {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        btn.disabled = false;
        btn.textContent = 'Alle Artikel neu taggen';
      }
    });
  }
}
