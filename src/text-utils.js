...
const COMPOUND_PREFIXES = new Set([
  'ab', 'an', 'auf', 'aus', 'be', 'bei', 'durch', 'ein', 'ent', 'er', 'ge', 'her', 'hin', 'mit',
  'nach', 'ueber', 'um', 'un', 'unter', 'ver', 'vor', 'weg', 'zu', 'zer'
]);
...
const GOOD_LEFT_ENDINGS = ['er', 'el', 'en', 'ung', 'heit', 'keit', 'schaft', 'tum', 'or', 'ar', 'ik'];
const GOOD_RIGHT_STARTS = ['premiere', 'auffuehrung', 'inszenierung', 'urauffuehrung', 'haus', 'buehne',
  'theater', 'stueck', 'oper', 'ballett', 'konzert', 'festival', 'foerderung', 'preis', 'fonds',
  'minister', 'stadt', 'land', 'haupt', 'meister', 'leitung', 'direktor', 'regie', 'kritik',
  'kultur', 'kunst', 'film', 'tanz', 'literatur', 'museum', 'sammlung', 'ausstellung', 'galerie'];

function scoreSplit(left, right, fuge) {
  let score = 0;
  if (GOOD_RIGHT_STARTS.includes(right)) score += 50;
  for (const start of GOOD_RIGHT_STARTS) {
    if (right.startsWith(start)) { score += 20; break; }
  }
  for (const end of GOOD_LEFT_ENDINGS) {
    if (left.endsWith(end)) { score += 10; break; }
  }
...
function germanCompoundSplit(word, { minWord = COMPOUND_MIN_WORD, minPart = COMPOUND_MIN_PART } = {}) {
...
function colognePhonetic(input) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    switch (c) {
      ...
      case 'd': case 't':
        code = (next === 'c' || next === 's' || next === 'z') ? '8' : '2'; break;
...
const LANG_MAP = {
  ...
  nld: 'nl', pol: 'pl', rus: 'ru', tur: 'tr', ces: 'cs', dan: 'da',
  swe: 'sv', nob: 'no', fin: 'fi', ell: 'el', ukr: 'uk', ron: 'ro',
  hun: 'hu', bul: 'bg', cat: 'ca', slv: 'sl', srp: 'sr', hrv: 'hr'
};
